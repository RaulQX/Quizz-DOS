...
namespace quizzdos_backend.Controllers
{
    ...
    public class CourseController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(PaginatedResponse<Course>), 200)]
        public async Task<ActionResult> GetCreatedCoursesPaged(Guid creatorId, int page = 1, int pageSize = 1)
        {
            var courses = await _courseRepository.GetCreatedCoursesPaged(creatorId, page, pageSize);
            return Ok(courses);
        }
        ...
        [ProducesResponseType(typeof(PaginatedResponse<Course>), 200)]
        [ProducesResponseType(typeof(string), 400)]
        public async Task<ActionResult> GetJoinedCoursesPaged(Guid personId, int page = 1, int pageSize = 1)
        {
            var courses = await _courseRepository.GetJoinedCoursesPaginated(personId, page, pageSize);
            if (courses == null)
